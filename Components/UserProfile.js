...
import ENV from '../env'; // Adjust as necessary for your environment variables
...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const response = await axios.get(`${ENV.API_BASE_URL}/api/auth/check-auth`, {
          headers: {
        ...
        if (response.data.success) {
          setProfile(response.data.user); // Set the profile data
        } else {
  ...
  const updateProfile = async () => {
    try {
      ...
      const response = await axios.post(`${ENV.API_BASE_URL}/api/auth/update-profile`, profile, {
        headers: {
